...
const BagsCard = () => {
  ...
  const showLoadMoreButton = visibleItems < BagsCardData.length;
  return (
    <>
      <div className={classes.title_section}>
        <div className={classes.title}>bags</div>
      </div>

      <div className={classes.bags_section}>
        {BagsCardData.slice(0, visibleItems).map((user) => (
          <Link to={`/bags/${user.id}`}>
